...
--судя по всему, из списка парсеров делает мега парсер, который парсит всё
...
-- read :: String -> a, т.е. приводит из стринга
int :: Parser String Token
int = some digit ||> val . C . read
--здесь с помощью lift мы игнорируем результат парсинга, и возвращаем свой парсер
...
tokenize s =
	case eof $ apply tokenizer s of 
		...
		_ -> error "ambigious tokenization"